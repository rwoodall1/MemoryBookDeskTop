...
namespace RESTModule {
    ...
    public class RESTService {
        ...
        private string EndPoint { get; set; } = "https://localhost:44333/api/xml/receiveXml";// =ConfigurationManager.AppSettings["MixBookEndPoint"].ToString();
        ...
        public RESTService() {
            ...
            try {
                ...
                using (var httpClient = new HttpClient(handler) { BaseAddress = new Uri(EndPoint) }) {
                    ...
                    var requestData = new StringContent(xmlRequestData,Encoding.UTF32, "application/xml");
                    ...
                    if (actionType.ToUpper() == "POST") {
                        ...
                        if (apiResponse.StatusCode == HttpStatusCode.Unauthorized) {
                            result.IsError = true;
                            result.Errors.Add(new ApiProcessingError("An error occurred API (Unauthorized). Please try again, if error persists contact support", "An error occurred calling  API (Unauthorized). Please try again, if error persists contact support", ""));
                        ...
                        result.IsError = true;
                            result.Errors.Add(new ApiProcessingError("An error occurred calling  API (Bad request). Please try again, if error persists contact support", "An error occurred calling  API (Bad request). Please try again, if error persists contact support", ""));
                        logData.ResponseStatusCode = apiResponse.StatusCode;
                        ...
                        logData.ResponseTimestamp = DateTime.Now;
                            ...
                            result.IsError = true;
                            result.Errors.Add(new ApiProcessingError("An error occurred calling  API (External API error). Please try again, if error persists contact support", "An error occurred calling  API (External API error). Please try again, if error persists contact support", ""));
                        logData.ResponseStatusCode = apiResponse.StatusCode;
