...
{
    public class TukiosScan : IScan
    {
        public TukiosScan(UserPrincipal userPrincipal)
        {
            Log = LogManager.GetLogger(GetType().FullName);
            // Constructor logic if needed
            ApplicationUser = userPrincipal;
        }
        protected TUKScanData data { get; set; }
        public UserPrincipal ApplicationUser { get; set; }
        string Invno { get; set; }
        protected Logger Log { get; set; }
        private TUKScanData scanData { get; set; }
        ...
        private TukiosBarScanModel TukModel { get; set; }
        public bool Scan(ScanData model)
        {
            this.data = model.TukiosScanData;
            this.Scanform = (frmBarScan)data.ScanForm;
            this.scanData = data;
            string vInvno = "";
            string _company = data.Barcode.ToUpper().Substring(0, 3);
            if (_company == "TUK")
            {
            ...
            string cmdText = @"
                            SELECT TO1.ShipName,
                            TO1.ProdInOrder,
                            (Select Max(ProdInOrder) from TukiosOrder where ClientOrderId=TO1.ClientOrderId)AS NumProducts,
                            TO1.ClientOrderId,
                            TO1.PrintergyFile,
                            TO1.BookId,
                            TO1.Invno,
                            ...
                            C.Specovr,
                            WD.MxbLocation AS BookLocation
                            From TukiosOrder 
                            TO1 Left Join Produtn P ON TO1.Invno=P.Invno
                            Left Join Covers C ON TO1.Invno=C.Invno
                            Left Join WipDetail WD On TO1.Invno=WD.Invno AND WD.DescripId IN (Select TOP 1 DescripId From wipdetail where  COALESCE(mxbLocation,'')!='' AND Invno=TO1.Invno  Order by DescripId desc )
            ...
            this.TukModel = (TukiosBarScanModel)result.Data;
            if (TukModel.TukiosOrderStatus != null && TukModel.TukiosOrderStatus.Trim().ToUpper() == "CANCELLED" || TukModel.TukiosOrderStatus != null && TukModel.TukiosOrderStatus.Trim().ToUpper() == "SHIPPED")
            {
                if (this.ApplicationUser.UserName.ToUpper() != "TRIMMING")
                {
                    MbcMessageBox.Information("This order has been " + TukModel.TukiosOrderStatus.ToUpper());

                    return false;
                }
            }
            this.Scanform.lblBkLocation.Text = TukModel.BookLocation;


            ////to impersonate finish later
            string currentUser = ApplicationUser.UserName.ToUpper();
            string _intitals = GetDepartmentInitials(data.Department.DeptCode);
                ...
                switch (currentUser)
                {
                    case "PRESS":
                        ...
                        sqlClient.CommandText(@"Update WIPDetail SET
                                                        WAR=@WAR,Schcode=@Schcode, WIR=@WIR,WTR=@WTR  WHERE Invno=@Invno AND DescripID=@DescripID");
                    ...
                    case "BINDING2":
                        ...
                        if (string.IsNullOrEmpty(Scanform.txtLocation.Text) && TukModel.Backing == "HC")
                        {
                        ...
                        sqlClient.AddParameter("@Schcode", "1");
                        sqlClient.CommandText(@"Update WIPDetail SET
                                                                WAR=@WAR,WIR = @WIR,WTR=@WTR,Schcode=@Schcode WHERE Invno=@Invno AND DescripID=@DescripID ");
                        ...
                        var wipUpdateresult = sqlClient.Insert();
                        if (wipUpdateresult.IsError)
                        {
                            MessageBox.Show("Failed to insert scan.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            Log.WithProperty("Property1", this.ApplicationUser.UserName).Error("Failed to insert scan.", wipUpdateresult.Errors[0].DeveloperMessage);
                            return false;
                        }
                        ...
                        if (TukModel.Backing == "HC")
                        {
                            ...
                            if (ConfigurationManager.AppSettings["Environment"].ToString() != "DEV")
                            {
                                ...
                                {
                                    MbcMessageBox.Hand("This options is not enabled. Contact Tammy Fowler if you need it.", "Print Label");
                                    //Print to labeler
                        ...
                        sqlClient.ClearParameters();
                        sqlClient.CommandText(@"Update TukiosOrder Set CurrentBookLoc=@CurrentBookLoc Where Invno=@Invno");
                        sqlClient.AddParameter("@Invno", this.Invno);
                    ...
                    case "BINDING":
                        ...
                        sqlClient.AddParameter("@Invno", this.Invno);
                        sqlClient.AddParameter("@BookStatus", "Binding");
                        var trimUpdateResult = sqlClient.Update();
                        if (trimUpdateResult.IsError)
                        {
                            Log.Error("Failed to update Order Status:" + trimUpdateResult.Errors[0].DeveloperMessage);
                        }
                    ...
                    case "QUALITY":
                        Scanform.lblHoldLocation.Text = "";//clear last scan
                                                           //war is datetime
                                                           //wir is initials
                                                           //PrintPackingList(TukModel.ClientOrderId);
                        vDeptCode = "50";
                        vWIR = "QY";
                        ...
                        sqlClient.AddParameter("@Invno", Invno);
                        sqlClient.CommandText(@"Select TukiosOrderStatus from TukiosOrder Where Invno=@Invno AND TukiosOrderStatus='Shipped'  ");
                        ...
                        sqlClient.CommandText(@"Select MO.ClientOrderId,MO.Invno,WD.MxbLocation,WD.DescripId 
                                                                FROM TukiosOrder MO Left Join WipDetail WD On MO.Invno=WD.invno
                        ...
                        {
                            var frmQH = new frmquailtyHold(TukModel.NumProducts, Scanform.lblHoldLocation.Text);
                            DialogResult holdresult = frmQH.ShowDialog();
                            if (holdresult == DialogResult.Yes)
                            {
                                location = frmQH.Location;
                                sqlClient.ClearParameters();
                                sqlClient.CommandText(@"Update TukiosOrder Set CurrentBookLoc=@CurrentBookLoc,CurrentCoverLoc=@CurrentCoverLoc Where Invno=@Invno");
                                sqlClient.AddParameter("@Invno", this.Invno);
                        ...
                        else { PrintPackingList(TukModel.ClientOrderId); }
                        sqlClient.ClearParameters();
                        sqlClient.AddParameter("@Invno", this.Invno);
                        sqlClient.AddParameter("@DescripID", vDeptCode);
                        sqlClient.AddParameter("@WAR", vDateTime);
                        ...
                        sqlClient.CommandText(@" IF NOT EXISTS (Select tmp.Invno,tmp.DescripID from WipDetail tmp WHERE tmp.Invno=@Invno and tmp.DescripID=@DescripID) 
                                                                    Begin
                                                                    INSERT INTO WipDetail (DescripID,War,Wir,WTR,MxbLocation,Invno) VALUES(@DescripID,@WAR,@WIR,@WTR,@MxbLocation,@Invno);
                                                                    END
                                                                    ");
                        ...
                        sqlClient.ClearParameters();
                        sqlClient.CommandText(@"Select MO.Invno,WD.MxbLocation From TukiosOrder MO
                ...
                switch (currentUser)
                {
                    case "ONBOARD":
                        ...
                        else
                        {
                            if (TukModel.Quantity > 1)
                            {
                                MbcMessageBox.Information("You should have " + TukModel.Quantity.ToString() + " copies in this order");
                            }
                        }

                        string input = Interaction.InputBox("Qty in Order:" + TukModel.Quantity.ToString(), "Assign Cover Location", "Enter A Location");
                        ...
                        sqlClient.CommandText(@"Update CoverDetail SET WAR= @WAR ,WTR=@WTR,Schcode=@Schcode, WIR =@WIR,MxbLocation=@Location  WHERE Invno=@Invno AND DescripID=@DescripID ");
                        var mxResult = sqlClient.Update();
                        if (mxResult.IsError)
                        {
                            MessageBox.Show("Failed to update scan.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        ...
                        sqlClient.CommandText(@" IF NOT EXISTS (Select tmp.Invno,tmp.DescripID from CoverDetail tmp WHERE tmp.Invno=@Invno and tmp.DescripID=@DescripID) 
                                                                    Begin
                                                                    INSERT INTO CoverDetail (DescripID,War,Wir,Wtr,Invno,MxbLocation,Schcode) VALUES(@DescripID,@WAR,@WIR,@WTR,@Invno,@Location,@Schcode);
                                                                    END
                                                                    ");
                        ...
                        {
                            MessageBox.Show("Failed to insert scan.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            Log.WithProperty("Property1", this.ApplicationUser.UserName).Error("Failed to insert scan:" + insertResult.Errors[0].DeveloperMessage);
                            return false;
                        }
                        sqlClient.ClearParameters();
                        sqlClient.CommandText(@"Update TukiosOrder Set CurrentCoverLoc=@CurrentCoverLoc Where Invno=@Invno");
                        sqlClient.AddParameter("@Invno", this.Invno);
                        ...
                        sqlClient.ClearParameters();
                        sqlClient.CommandText(@"Update TukiosOrder Set CoverStatus=@BookStatus where Invno=@Invno");
                        sqlClient.AddParameter("@Invno", this.Invno);
                    ...
                    case "TRIMMING":
                        ...
                        sqlClient.CommandText(@"Update CoverDetail SET WAR= @WAR ,WIR =@WIR ,WTR=@WTR  WHERE Invno=@Invno AND DescripID=@DescripID ");
                        var mxResult1 = sqlClient.Update();
                        if (mxResult1.IsError)
                        {
                            MessageBox.Show("Failed to insert scan.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        ...
                        sqlClient.CommandText(@" IF NOT EXISTS (Select tmp.Invno,tmp.DescripID from CoverDetail tmp WHERE tmp.Invno=@Invno and tmp.DescripID=@DescripID) 
                                                                    Begin
                                                                    INSERT INTO CoverDetail (DescripID,War,Wir,WTR, Invno) VALUES(@DescripID,@WAR,@WIR,@WTR,@Invno);
                                                                    END
                                                                    ");
                    ...
                    case "PRESS":
                        ...
                        sqlClient.AddParameter("@Schcode", "1");
                        sqlClient.CommandText(@"Update CoverDetail SET WAR= @WAR , WIR =@WIR ,WTR=@WTR,Schcode=@Schcode  WHERE Invno=@Invno AND DescripID=@DescripID ");
                        var mxResult2 = sqlClient.Update();
                        if (mxResult2.IsError)
                        {
                            MessageBox.Show("Failed to insert scan.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        ...
                        sqlClient.CommandText(@" IF NOT EXISTS (Select tmp.Invno,tmp.DescripID from CoverDetail tmp WHERE tmp.Invno=@Invno and tmp.DescripID=@DescripID) 
                                                                Begin
                                                                INSERT INTO CoverDetail (DescripID,War,Wir,WTR,Invno,Schcode) VALUES(@DescripID,@WAR,@WIR,@WTR,@Invno,@Schcode);
                                                                END
                                                                ");
        ...
        public bool ScanRemake()
        {
            ...
            if (ApplicationUser.UserName.ToUpper() == "QUALITY")
            {
                ...
                sqlClient.ClearParameters();
                sqlClient.CommandText(@"Update TukiosOrder SET CoverStatus='',BookStatus='',CurrentBookLoc='',CurrentCoverLoc='',RemakeTicketPrinted=0 where Invno=@Invno");
                sqlClient.AddParameter("@Invno", Invno);
        ...
        public bool ScanCheck(int invno, string login, string type)
        {
            ...
            bool retval = true;
            var sqlClient = new SQLCustomClient();
            sqlClient.CommandText(@"Select TukiosOrderStatus  FROM TukiosOrder Where Invno=@Invno ");
            sqlClient.AddParameter("@Invno", invno);
