...
Values( 
            ...
            foreach (var order in jpixOrders.JostensPIXOrder)
            {
                ...
                sqlClient.AddParameter("@Document", "\\\\sedsujpisl01\\workflow\\JPixFlyers\\Archive\\" + jpixOrders.RequestId + "\\" + order.Document);
                sqlClient.AddParameter("@NeedsByDate", order.DateNeedsByDate);
                sqlClient.AddParameter("@ProjectedShipDate", order.DateNeedsByDate.AddDays(-4));
                sqlClient.AddParameter("@ProductType", order.ProductType);
                sqlClient.AddParameter("@Quantity", order.Quantity);
                sqlClient.AddParameter("@ShipToContact", order.ShipToContact);
                sqlClient.AddParameter("@ShipToCustomerName", order.ShipToCustomerName);
                sqlClient.AddParameter("@ShipToAddress1", order.ShipToAddress1);
                sqlClient.AddParameter("@ShipToAddress2", order.ShipToAddress2);
                sqlClient.AddParameter("@ShipToCity", order.ShipToCity);
                sqlClient.AddParameter("@ShipToStateOrProvince", order.ShipToStateOrProvince);
                sqlClient.AddParameter("@ShipToPostalCode", order.ShipToPostalCode);
                sqlClient.AddParameter("@ShipToCountry", order.ShipToCountry);
                sqlClient.AddParameter("@ShipToGroup", order.ShipToGroup.ToString());
                sqlClient.AddParameter("@Reference", order.Reference);
                sqlClient.AddParameter("@DateReceived", DateTime.Now);
                sqlClient.AddParameter("@OracleCode", order.SchoolCode);
                sqlClient.AddParameter("@EventCode", order.EventCode);
        ...
        private async Task<ApiProcessingResult> InsertProduction(JostensPIXFulfillmentRequestsJostensPIXOrderItem order, string invno)
        {
            var processingResult = new ApiProcessingResult();
            ...
            sqlClient.AddParameter("@NoCopies", order.Quantity);
            sqlClient.AddParameter("@KitRecvd", DateTime.Now);
            sqlClient.AddParameter("@Prshpdte", order.DateNeedsByDate);
            var insertResult = sqlClient.Insert();
            if (insertResult.IsError)
            {
                Log.Error("Failed to insert produtn record for JPIX document:" + order.Document + " Invno:" + invno.ToString() + "|" + insertResult.Errors[0].DeveloperMessage);
                MbcMessageBox.Error("Failed to insert produtn record for document:" + order.Document + " Invno:" + invno.ToString() + "|" + insertResult.Errors[0].DeveloperMessage);
            }
            sqlClient.ClearParameters();
            ...
            {
                Log.Error("Failed to insert wip record for JPXIX document:" + order.Document + " Invno:" + invno.ToString() + "|" + insertResult.Errors[0].DeveloperMessage);
                MbcMessageBox.Error("Failed to insert wip record for document:" + order.Document + " Invno:" + invno.ToString() + "|" + insertResult.Errors[0].DeveloperMessage);
            }
            return processingResult;
        }
        ...
        private async Task SetFoldersAndFiles(JostensPIXFulfillmentRequests jpixOrders, string xmlName)
        {
            ...
            foreach (var order in jpixOrders.JostensPIXOrder)
            {
                try
                {
                    ...
                    string fileToCopy = dropPath + order.Document;
                    string destinationDirectory = workpath + order.Document;
                    File.Copy(fileToCopy, destinationDirectory, true);

                    File.Copy(fileToCopy, newFolder + "\\" + order.Document, true);
                    File.Delete(fileToCopy);

                }
                catch (Exception ex)
                {
                    MbcMessageBox.Error("Failed to copy file:" + order.Document + " RequestId:" + jpixOrders.RequestId.ToString() + "|" + ex.Message);
