...
{
    public partial class frmTKInvoiceReport : BaseClass.frmBase
    {
        public frmTKInvoiceReport(UserPrincipal userPrincipal, frmMain parent) : base(new string[] { "SA", "Administrator", "MixBook" }, userPrincipal)
        {
            InitializeComponent();
        ...
        private void RunReport()
        {
            ...
            string cmd = @"Select TO1.ClientOrderId
,TO1.Invno
,TO1.TukiosOrderStatus   
,TO1.OrderReceivedDate
...
,TO1.ShipState
,TO1.ShipZip
,''''+ Convert(VARCHAR, TO1.TrackingNumber) AS TrackingNumber
,MS.Cost As Freight
,MP.SellPrice As UnitPrice 
,MP.SellPrice * TO1.Copies AS UnitTotal
,MP.PerPage * (TO1.Pages * TO1.Copies )AS PageFee
,MP.HandlingPerBox AS Fulfillment
,(MP.SellPrice * TO1.Copies)+(MP.PerPage * (TO1.Pages * TO1.Copies ))+(MP.HandlingPerBox) AS Total
FROM TukiosOrder TO1 INNER JOIN MixBookPricing MP ON TO1.ItemCode=MP.ItemCode
Left Join TukiosShipping MS ON TO1.ClientOrderId=MS.ClientOrderId
Where TO1.TukiosOrderStatus='Shipped' and (OrderReprint=0 OR OrderReprint IS NULL) and (Invoiced IS NULL OR Invoiced =0)And (TO1.DateShipped >= @DateFrom And TO1.DateShipped <= @DateTo)  Order By DateShipped,Invno";
...
,MP.HandlingPerBox AS Fulfillment
,(MP.SellPrice * TO1.Copies)+(MP.PerPage * (TO1.Pages * TO1.Copies ))+(MP.HandlingPerBox) AS Total
FROM TukiosOrder M INNER JOIN MixBookPricing MP ON M.ItemCode=MP.ItemCode
Left Join TukiosShipping MS ON M.ClientOrderId=MS.ClientOrderId
